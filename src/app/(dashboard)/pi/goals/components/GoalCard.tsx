...
const statusColors: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  archived: { bg: "bg-gray-500/15", text: "text-sub", dot: "bg-gray-500" },
};
...
export default function GoalCard({ goal, onTap }: GoalCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={() => onTap(goal)}
      className="w-full h-full flex flex-col justify-between rounded-2xl border border-edge bg-card p-5 text-left transition-colors active:bg-elevated active:border-accent/50"
    >
      {/* Top: Status badge */}
      ...
      {/* Middle: Title */}
      <h3 className="text-lg font-bold text-heading leading-snug line-clamp-2 flex-1">
        {goal.title}
      </h3>

      {/* Bottom: Description preview */}
      <p className="text-sm text-dim line-clamp-2 mt-2">
