    ...
    <div className="h-[720px] w-[1280px] mx-auto flex flex-col bg-[#0a0a0f] overflow-hidden select-none">
      ...
      <header className="flex items-center justify-between px-8 py-5 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-heading">Goals</h1>
        ...
        <button
          ...
          className="flex items-center gap-3 px-6 py-4 rounded-xl bg-gradient-to-r from-accent to-accent-secondary text-white text-lg font-semibold active:opacity-80 transition-opacity"
        >
      ...
      <div className="flex-1 px-8 pb-6 min-h-0">
        {loading ? (
          ...
          <div className="h-full flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-base text-dim">Tap &quot;Add Goal&quot; to get started</p>
          </div>
        ) : (
          <div className="h-full grid grid-cols-3 grid-rows-2 gap-4">
            ...
            {Array.from({ length: Math.max(0, 6 - visibleGoals.length) }).map((_, i) => (
              <div
                key={`empty-${i}`}
                className="rounded-2xl border border-dashed border-edge bg-surface/50 flex items-center justify-center"
              >
                <button
                  type="button"
                  onClick={() => setShowAdd(true)}
                  className="flex flex-col items-center gap-2 text-dim active:text-sub transition-colors p-6"
                >
